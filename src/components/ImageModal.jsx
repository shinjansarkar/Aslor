...
export default function ImageModal({ src, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md transition-all">
      <div className="relative max-w-5xl w-full bg-[#071710] rounded-2xl border border-emerald-500/40 p-4 overflow-hidden shadow-2xl space-y-3">
        
        {/* Modal Top Bar */}
        <div className="flex items-center justify-between border-b border-emerald-900/60 pb-3">
          <div className="flex items-center gap-2">
            <img src="/logo.webp" alt="Logo" className="w-6 h-6 object-contain" />
            <span className="text-sm font-bold text-white">ASLOR ENTERPRISES • Solar Solutions Guide</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={onClose}
              className="p-1.5 rounded-lg bg-emerald-950 hover:bg-emerald-900 text-slate-300 hover:text-white border border-emerald-800"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
